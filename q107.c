/*Write a program to take an array arr[] of integers as input, the task is to find the previous
 ...
 than the current element. If there does not exist next greater of current element, then previous greater 
...
#include <stdio.h>
#define MAX_SIZE 100
void findPreviousGreaterElements(int arr[], int n, int result[]) {
    for (int i = 0; i < n; i++) {
        result[i] = -1; 
        for (int j = i - 1; j >= 0; j--) {
            if (arr[j] > arr[i]) {
